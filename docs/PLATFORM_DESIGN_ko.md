...
- AI는 기존 `lib/ai.ts` 패턴 재활용 — **OpenAI 사용(확정)**. (키 미설정 시 규칙기반 초안으로 폴백)
- 기본 문체/길이(조정 가능): **총평 3~4문장(분석+방향)**, **개별 코멘트 5~7문장(격려+구체 방향, 반영 키워드 활용)**.
...
- [x] Ⓒ 범용형 성적표: 표준점수 성장추이 + 담임 의견(총평+개별, 긍정 키워드 칩) — **확정**
- [x] 담임 의견 AI: **OpenAI 사용**, 기본 문체/길이 지정(§6) — **확정**
- [x] 국영수 Ⓐ: 기존 `exams.json` 정답키 재활용(수학 단답형은 v2) — **확정**
- [ ] 구현 착수 시 mock-report-web **쓰기 권한 연결 + PR** 진행 동의 ← 다음 단계

> 설계 v1 확정. 다음: Phase A(기반+범용) 구현.
